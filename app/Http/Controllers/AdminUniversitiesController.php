...
use Illuminate\Support\Facades\DB as FacadesDB;
	...
	class AdminUniversitiesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Nomor telepon","name"=>"phone_number"];
			...
			$this->form[] = ['label'=>'Nomor telepon','name'=>'phone_number','type'=>'number','validation'=>'required|numeric','width'=>'col-sm-10','placeholder'=>'You can only enter the number only'];
			$this->form[] = ['label'=>'Email','name'=>'email','type'=>'email','validation'=>'required|min:1|max:255|email|unique:universities','width'=>'col-sm-10','placeholder'=>'Please enter a valid email address'];
			...
			//$this->form[] = ["label"=>"Email","name"=>"email","type"=>"email","required"=>TRUE,"validation"=>"required|min:1|max:255|email|unique:universities","placeholder"=>"Please enter a valid email address"];
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
			...
			$previllege = CmsPrivileges::where('name', '=', 'Universitas')->first();
			...
			// $user->university_id = $postdata['university_id'];
			$user->save();
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {        
			...
			if(isset($postdata['password']) && !is_null($postdata['password'])) {
				CmsUsers::where('university_id', '=', $id)
					->update([
