...
use Illuminate\Support\Facades\DB as FacadesDB;
	...
	class AdminCompaniesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Email Perusahaan","name"=>"email"];
			$this->col[] = ["label"=>"Bidang","name"=>"bussiness_field"];
			$this->col[] = ["label"=>"Lokasi","name"=>"location"];
			$this->col[] = ["label"=>"Tentang Kami","name"=>"about"];
			...
			$this->form[] = ['label'=>'Email Perusahaan','name'=>'email','type'=>'text','validation'=>'required|min:1|max:255|email','width'=>'col-sm-10', "help"=>"Email akan digunakan untuk login."];
			$this->form[] = ['label'=>'Bidang','name'=>'bussiness_field','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Lokasi','name'=>'location','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'TAG','name'=>'tag','type'=>'checkbox','datatable'=>'tags,name','relationship_table'=>'tags_companies'];
			$this->form[] = array("label"=>"Kata Sandi","name"=>"password","type"=>"password","help"=>"Please leave empty if not change");
			// $this->form[] = ['label'=>'TAG','type'=>'select2','datatable'=>'companies,name','relationship_table'=>'tags_companies'];
			$this->form[] = ['label'=>'Tentang Kami','name'=>'about','type'=>'wysiwyg','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
	        ...
	        */
			...
			$this->sub_module[] = ['label'=>'TAG','path'=>'tags_companies','parent_columns'=>'name','foreign_key'=>'companies_id','button_color'=>'success','button_icon'=>'fa fa-bars'];
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
			...
			$previllege = CmsPrivileges::where('name', '=', 'Perusahaan')->first();
			...
			$user->type = 'company';
			// $user->company_id = $postdata['company_id'];
			$user->save();
